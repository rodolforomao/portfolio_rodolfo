...
import SystemOS from "./components/SystemOS/SystemOS";
import {
...
function AppShell() {
  ...
  const isSystemRoute = location.pathname === "/";
  ...
  useEffect(() => {
    document.body.classList.toggle("system-route-active", isSystemRoute);
    return () => document.body.classList.remove("system-route-active");
  }, [isSystemRoute]);
  ...
  return (
    <>
      ...
      {!isDealerRoute && !isSystemRoute && <Footer />}
    </>
  );
}
