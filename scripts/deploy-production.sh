...
#   4. liquid_pots_server.py + nginx        (deploy-liquid-pots.sh)
#
...
#   DEPLOY_LIQUID_POTS_PATH , DEPLOY_NGINX_CONF_DIR , REACT_APP_DEALER_* , VAULT_ADMIN_TOKEN
#   TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID  — mensageria única (Dealer + potes)
#   DEPLOY_WITH_VAULT=0          — pula vault/relay (só frontend)
#   DEPLOY_WITH_PORTFOLIO_API=0  — pula portfolio-api
#   DEPLOY_WITH_LIQUID_POTS=0    — pula liquid-pots
...
echo "▶ [1/4] Frontend React → $DEPLOY_REMOTE_PATH"
bash "$SCRIPT_DIR/deploy-ssh.sh"
echo

if [[ "${DEPLOY_WITH_VAULT:-1}" == "1" ]]; then
  echo "▶ [2/4] Vault + relay + nginx"
...
if [[ "${DEPLOY_WITH_LIQUID_POTS:-1}" == "1" ]]; then
  echo "▶ [4/4] Liquid pots API + nginx + Telegram"
  bash "$SCRIPT_DIR/deploy-liquid-pots.sh"
else
  echo "▸ Liquid pots ignorado (DEPLOY_WITH_LIQUID_POTS=0)"
  echo "  Rode: bash scripts/deploy-liquid-pots.sh"
  bash "$SCRIPT_DIR/verify-liquid-pots-prod.sh" "https://${PUBLIC_HOST}" || true
fi
